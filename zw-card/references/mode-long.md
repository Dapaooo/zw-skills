...
本模具产出的是「阅读卡」，**不是转录**。务必先通读原文、抽取高信号结构并**压缩**再上版；直接照搬逐字稿是最常见的失败模式。

- **提炼骨架**：保留标题、章节标题（##/###）与支撑论点的关键段落；删去寒暄、重复、以及来回问答中的冗余铺陈。
- **识别金句**：独立成段、< 25 字、承载核心洞察的短句，用 `.highlight` 渲染（见步骤 3）。
- **识别引用 / 加粗**：原文中已被作者加粗或作为引述的高价值句，优先保留为 `blockquote` 或 `<strong>`。
- **按空行分割**为段落列表，准备格式化。
- **不做切分（版式含义）**：指「所有内容放进一张长卡」，与 `-m` 多卡相对；**绝不意味着逐字保留原文**。长文铸成卡后正文篇幅应约为原文的 1/3 ~ 1/2；若几乎与原文等长，说明未提炼，需重做。

> 自检：铸出的卡读起来应像「有人帮你读完并划了重点」，而不是「复制粘贴全文」。
...
- 普通段落 → `<p>文本</p>`
- 大章节标题（## 级别） → `<h2>标题</h2>`
- 子章节标题（### 级别、带编号的小标题） → `<h3>标题</h3>`
- 引用 → `<blockquote><p>引用</p></blockquote>`
- 加粗 → `<strong>文本</strong>`
- 列表 → `<ul><li>...</li></ul>`

**层级判断（关键，极易混淆）：**
- `<h2>`：文章的一级内容章节（如"智能体技术走到哪了""这对行业意味着什么"）
- `<h3>`：章节内的编号子标题或独立小节标题（如"1. 从能想到能做""从可用到好用"）——**这是内容标题，必须用 h3**
- `.subtitle`：**仅用于装饰性标签**（来源信息、分类标签、日期等非内容性文字），**绝不用于内容标题**
...
**副标题标签（仅限装饰性文字，禁止用于内容标题）：**
```html
<p class="subtitle">标签文字</p>
```
用途：来源信息（"腾讯研究院 · 企业管理杂志"）、分类标签、日期等。**内容性子标题必须用 `<h3>`，不得用 `.subtitle`。**
